...
First collection of objects contains 15 objects with assigned colours (red,
green or blue). Naming scheme reflects a colour and a group, for example 'r1' is
...
Real objects were 3D printed in specific colours following this name convention.
...
A total of 158 objects are in the dataset.
...
_Singleton = mesh_object.Singleton
...
class PropsDatasetType:
  ...
  version: PropsVersion  # RGB-object version.
  ids_list: List[str]  # List of object names.
  mesh_paths: List[str]  # List of directories to fetch mesh files from.
...
DEFORMATION_AXES = (
    DEFORMATION_TRAIN_AXES + DEFORMATION_VALIDATION_AXES)


def _define_deformation_axes() -> Dict[str, List[str]]:
...
PROP_FEATURES = {
    PropsVersion.RGB_OBJECTS_V1:
        PropsDatasetType(
            version=PropsVersion.RGB_OBJECTS_V1,
            ids_list=RGB_OBJECTS_FULL_SET,
...
def random_set(
    rgb_version: PropsVersion = PropsVersion.RGB_OBJECTS_V1,
    id_list: Optional[List[str]] = None,
    id_list_red: Optional[List[str]] = None,
    id_list_green: Optional[List[str]] = None,
    id_list_blue: Optional[List[str]] = None) -> Tuple[PropsVersion, List[str]]:
  ...
  desired colour in the constructor.
  ...
  if id_list:
    for object_id in id_list:
      if object_id not in PROP_FEATURES[rgb_version].ids_list:
        raise ValueError(f'id_list includes {object_id} which is not part of '
                         f'{rgb_version}')
  else:
    id_list = PROP_FEATURES[rgb_version].ids_list
  if not id_list_red:
    id_list_red = id_list
  ...
  prop_ids.append(np.random.choice(id_list_red, 1)[0])
  prop_ids.append(np.random.choice(id_list_green, 1)[0])
  prop_ids.append(np.random.choice(id_list_blue, 1)[0])
  return (rgb_version, prop_ids)


def fixed_random_set(
    rgb_version: PropsVersion = PropsVersion.RGB_OBJECTS_V1
) -> Tuple[PropsVersion, List[str]]:
  ...
  Returns:
    Set of 3 props from a predefined set.
  """
  if rgb_version == PropsVersion.RGB_OBJECTS_V1:
    props = np.random.choice([
        s for s in PROP_SETS_TRAINING if s.startswith('rgb_test_set')
    ])
    return (PROP_SETS_TRAINING[props][0],
            list(PROP_SETS_TRAINING[props][1]))
  else:
    raise ValueError(
...
def _define_blue_prop_sets(
  ...
  for a in axes:
    ...
    blue_obj_sets[f'{base_str}{a}'] = functools.partial(
        random_set,
        rgb_version=PropsVersion.RGB_OBJECTS_V1,
        id_list=id_list,
...
PROP_SETS_TRAINING = {
    ...
    'rgb_test_set1': (PropsVersion.RGB_OBJECTS_V1, ('r3', 'd2', 'b2')),
    'rgb_test_set2': (PropsVersion.RGB_OBJECTS_V1, ('r5', 'g2', 'b3')),
    'rgb_test_set3': (PropsVersion.RGB_OBJECTS_V1, ('r6', 'g3', 'b5')),
    'rgb_test_set4': (PropsVersion.RGB_OBJECTS_V1, ('d2', 'g5', 'b6')),
    'rgb_test_set5': (PropsVersion.RGB_OBJECTS_V1, ('r2', 'g6', 'd2')),
}

_BLUE_PROP_SETS = _define_blue_prop_sets()
_BLUE_PROP_SETS_TRAIN = _define_blue_prop_sets(
    base_str='rgb_train_blue_dim',
    id_list=RGB_OBJECTS_TRAIN_SET,
    axes=DEFORMATION_TRAIN_AXES)
_BLUE_PROP_SETS_VALIDATION = _define_blue_prop_sets(
    base_str='rgb_validation_blue_dim',
    id_list=RGB_OBJECTS_VALIDATION_SET,
    axes=DEFORMATION_VALIDATION_AXES)

RANDOM_PROP_SETS_FUNCTIONS = mesh_object.PropSetDict({
    # Return changing sets on every access.
    'rgb_random':
        functools.partial(
            random_set,
            rgb_version=PropsVersion.RGB_OBJECTS_V1,
            id_list=RGB_OBJECTS_FULL_SET),
    'rgb_train_random':
        functools.partial(
            random_set,
            rgb_version=PropsVersion.RGB_OBJECTS_V1,
            id_list=RGB_OBJECTS_TRAIN_SET),
    'rgb_validation_random':
        functools.partial(
            random_set,
            rgb_version=PropsVersion.RGB_OBJECTS_V1,
            id_list=RGB_OBJECTS_VALIDATION_SET),
    'rgb_test_random':
        functools.partial(
            random_set,
            rgb_version=PropsVersion.RGB_OBJECTS_V1,
            id_list=RGB_OBJECTS_TEST_SET),
    'rgb_test_fixed_random':
        functools.partial(
            fixed_random_set, rgb_version=PropsVersion.RGB_OBJECTS_V1),
    **_BLUE_PROP_SETS,
    **_BLUE_PROP_SETS_TRAIN,
    **_BLUE_PROP_SETS_VALIDATION
})

PROP_SETS = mesh_object.PropSetDict({
    **PROP_SETS_TRAINING,
    **RANDOM_PROP_SETS_FUNCTIONS,
})


def _get_all_meshes(
    rgb_version: PropsVersion = PropsVersion.RGB_OBJECTS_V1) -> List[str]:
